...
df <- rio::import(BirdNET_selection_table)

df <- df %>%
  dplyr::mutate(bins = cut(Confidence, breaks = seq(0, 1, length.out = 11), labels = FALSE, include.lowest = TRUE)/10)

plotheight <- length(unique(df$`Common Name`))/10

df <- df %>%
  dplyr::group_by(`Common Name`) %>%
  dplyr::summarise(mean = mean(Confidence, na.rm = TRUE), .groups = "drop") %>%
...
ggplot2::ggsave(fig1, filename = file.path(dirname.target.figures, "speciesBins.png"), height = plotheight)
...
ggplot2::ggsave(fig2, filename = file.path(dirname.target.figures, "speciesConfidence.png"), height = plotheight)
}
